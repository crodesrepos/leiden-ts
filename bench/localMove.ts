/**
 * Local-moving phase benchmark. Cat-4 Budget per `docs/test-philosophy.md`.
 * Asserts against `bench/baselines.json` and exits non-zero on regression.
 */
...
function main(): number {
  ...
  console.log(
    `\n# leiden-ts localMove benchmark — ${new Date().toISOString()}\n`,
  );
  console.log(
  ...
  for (const { n, m } of cases) {
    ...
    const label = `localMove n=${n} m=${m}`;

    // Warm + measure. Smaller run count than modularity bench because
    // localMove is significantly more expensive per call.
    // ER graphs converge slowly because they lack strong community
    // structure. The cap of 1000 is well above what real workloads need;
    // for community-rich graphs (LFR, real social networks) localMove
    // converges in 5–20 iterations.
    const result = bench(
      label,
      () => {
        localMove(graph, undefined, { seed: 1, maxIterations: 1000 });
      },
      { warmup: 2, runs: 10 },
    );
    ...
    if (budget !== undefined) {
      ...
      if (ratio > ratioMax) {
        failures.push(
          `❌ BUDGET p99/p50: ${label}: ratio=${ratio.toFixed(2)} > ${ratioMax} (suggests GC pauses)`,
        );
      }
    } else {
      console.log(
